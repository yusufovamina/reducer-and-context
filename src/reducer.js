...
export function reducer(state, action){
    if(action.type === 'ADD TO BAG'){
       let newArrGoods = [...state.goods]
       let newArrBag = [...state.bag]
       let obj = newArrGoods[action.index]
       newArrBag = [...state.bag, obj]
       let count = newArrBag.length
       console.log(state.bag)
       console.log(state.bag.length)
       return {...state, bag:[...newArrBag], count:count}
    }
